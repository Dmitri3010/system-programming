...
extern "C++" __declspec(dllexport) void OpenExcel(wchar_t* text)
{
							...
							if (SUCCEEDED(hr))
							{
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										CallArgs[0].bstrVal = SysAllocString(L"A4");
										DispParams.rgvarg = CallArgs;
										...
										if (SUCCEEDED(hr))
										{
											...
											if (SUCCEEDED(hr))
											{
												...
												CallArgs[0].bstrVal = SysAllocString(text);  //Try to set data to cell A1 using pXLRange
												DispParams.rgvarg = CallArgs;
												DispParams.rgdispidNamedArgs = &dispidNamed;
